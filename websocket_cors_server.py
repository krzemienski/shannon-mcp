#!/usr/bin/env python3
"""
WebSocket server with HTTP CORS support for Shannon MCP Server
"""
...
async def message(sid, data):
    ...
    try:
        ...
        await sio.emit('response', error_response, room=sid)

async def handle_tool_request(tool_name: str, params: dict) -> dict:
...
async def index(request):
    return web.Response(text='Shannon MCP WebSocket Server', content_type='text/plain')

# Add routes
root_route = app.router.add_get('/', index)
cors.add(root_route)

if __name__ == '__main__':
    logger.info("Starting Shannon MCP Socket.IO Server on port 8765")
